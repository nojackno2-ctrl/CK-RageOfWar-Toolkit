...
- **單一執行檔、單一主視窗**：使用者面對的只有一個執行檔 `CKToolkit.exe`，主 GUI 只有一個視窗。
  不得再拆出第二個工具——第二個 exe、第二套安裝流程、獨立的輔助程式，都不行。
  > **輔助視窗例外（使用者決定，2026-08-31）**：同一個行程可以開啟**輔助性的置頂小視窗**，
  > 供遊戲執行中即時操作（修改器面板）。條件是：由主視窗自己開關、關掉後不留任何常駐、
  > 不另外發佈成獨立執行檔、也不重複主視窗已有的設定功能——它只是既有功能的遙控器。
  > 成因是引擎只認 20 個硬編按鍵且派送不看修飾鍵（見 `ISSUES.md` ISSUE-053 / ISSUE-054），
  > 筆電沒有小鍵盤又不想用 F1~F12 時幾乎無鍵可綁，滑鼠面板是唯一的出路。
...
### 2.9 修改器對執行中遊戲的兩項例外（使用者決定，2026-08-31 / 2026-09-02）

修改器的**持久性**改動仍然全部是檔案修補：數值 Tweaks 與按鍵表寫進遊戲檔案，
不常駐、關掉工具就只剩檔案。以下兩項是對「執行中的遊戲行程」的例外，
**範圍以本節列出的為準，不得擴大**。要新增任何其他記憶體讀寫或行程內行為，
必須先在此處增列，不得默默擴張。

#### 例外一：生成位置選點（2026-08-31）

見下一段的完整條款。

#### 例外二：執行期腳本通道（2026-09-02，ISSUE-068）

**成因**：引擎只認 20 個硬編的 scdebug 按鍵代號，9 個被遊戲自己用掉、5 個被原版
scdebug 綁走，只剩 4 個自由鍵；小鍵盤模式雖然把 F1–F12 解放成 13 個，但那 13 個
對映到的是筆電沒有的實體小鍵盤。18 個作弊塞不進去，塞不進去的就被靜默停用，
面板連按鈕都不會出現。使用者實測的結論是「根本沒鍵可按」——修改器等於改不到遊戲。

按鍵其實只是「把一段字串交給引擎腳本編譯器」的其中一種方式。因此允許
`ckperf.dll`（`src/CKPerf/script.cpp`）在遊戲行程內重現 scdebug 派送的尾段，
由 CKToolkit 經具名管線送入作弊腳本原文：

- **只呼叫引擎自己的函式**，不注入任何自製機器碼、不改寫任何指令：
  `0x005E0340` 編譯 → `0x005E0430` 同步執行（或 `0x005E1D70` 排程）→
  `0x0041B480` 的 vtable`+0x0C` 釋放。完整位址表與反組譯依據見
  `docs/reverse-engineering-notes.md`「引擎腳本執行鏈」一節。
- **對不上就拒絕**：安裝時逐一比對七個進入點的原始位元組簽章（含 scdebug 派送
  call site 與 `"void"` 字面量）。任一不符就**永久停用整條通道**並寫進 `ckperf.log`，
  絕不猜測、絕不部分啟用。
- **上線前自證**：`ScriptChannelSelfTest()` 先編譯一段無副作用腳本（`int i; i = 1;`）
  並在**不執行**的情況下釋放。編不過就停用通道。紀律比照 `CrashSelfTest`／`NullStoreSelfTest`。
- **必須在對局中**：派送前確認遊戲主物件、主控台與腳本 VM 排程器都解得出合理指標，
  否則 fail-closed 回報「不在對局中」，一個位元組都不動。
- **只在主執行緒執行**：抽取點是 `frames.cpp` 的 `SetDIBitsToDevice` hook，
  與引擎自己執行按鍵腳本的執行緒相同。不得從管線執行緒直接呼叫引擎。
- **完全不碰磁碟**：通道只活在行程內，遊戲結束就什麼都不剩，因此不涉及 §2.1／§2.3
  的可逆性義務。作弊因此**不再需要寫進 `SCDEBUG.XML`**。
- **要驗證身分**：管線以目前使用者的預設 DACL 建立、拒絕遠端連線，並附帶一組
  每次注入現產的 32 字元權杖（`ScriptChannel.NewToken`）。權杖不符一律丟棄並記錄；
  權杖長度不對時原生端連通道都不會開。權杖不寫檔、不進記錄檔。
- **腳本內容只能是本工具產生的作弊腳本**，與 `SCDEBUG.XML` 路徑共用同一個
  `Cheat.Script` builder。兩條路徑對同一個作弊必須逐字相同，SelfTest 第 46 組鎖住這條。

「在滑鼠位置生成單位／物品」的座標取自 VS 函式 `MousePtm()`（handler 在 .text VA
`0x005CBD40`）。它不呼叫 `GetCursorPos`，只是把 `[[0x008AAB80] + 0x20]` 這 8 個位元組
推進 VM 堆疊，而那是一個由滑鼠移動訊息更新的快取。使用者透過「遊戲中面板」代按熱鍵時，
游標從目標點移到按鈕的路上會一路更新它，單位就生成在面板邊緣而不是使用者要的位置。

因此允許 `Core/Runtime/GameMemory.cs` 對執行中的遊戲行程做**唯一一種**記憶體存取：

- 只有這 8 個位元組（`[[0x008AAB80] + 0x20]` 的 x/y），不得讀寫任何其他位址。
- 只寫**引擎自己算出來的值**——游標停在目標上時讀下來、按下按鈕時原樣寫回。
  不得自行推導螢幕像素到地圖座標的換算後寫入。
- **走例外二的腳本通道時只讀不寫**：座標讀下來之後直接當字面值放進腳本
  （`Point(x, y)`，用法與遊戲自己的 `SUBAI\BARRACK_TRAIN.VS` 相同），
  不必再寫回快取。寫入只保留給通道不可用、退回代送按鍵的備援路徑。
- 只在「遊戲中面板」開著、而且確實有啟用游標位置類作弊時才建立連線
  （`Cheats.CursorPositionCheats` 是唯一的清單來源）。
- **完全不碰磁碟**，關掉面板就什麼都不剩，因此不涉及 §2.1 的可逆性義務。
- 寫入前必須核對 `MousePtm` handler 開頭的 `A1 80 AB 8A 00 8D 48 20`，
  並確認全域指標落在合理的使用者位址空間；任一項對不上就整條路徑停用、
  退回倒數模式，絕不亂寫（§2 的「對不上就拒絕」在這裡同樣適用）。
- 位址一律以目標行程的模組基底換算，不得寫死絕對位址。

要新增任何其他記憶體讀寫，必須先在此處增列，不得默默擴張。
