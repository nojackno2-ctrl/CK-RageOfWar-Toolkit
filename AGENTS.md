...
> **玩家存檔例外（使用者新增功能，2026-08-23）**：`profiles\<玩家>\*.adv` 與同名
> `.adv.bmp` 是使用者資料，不是 Steam 原廠遊戲內容。使用者主動要求的存檔管理可將這兩類
> 檔案封裝成 `.cksave`，並可在保護性刪除前存入 `%LocalAppData%\CKToolkit\SaveTrash`。
> 此例外不包含 `player.ini`、`profiles.ini`、`currentadv.bfhp`、`Adventures/`、`Scenarios/`
> 或任何 EXE／PAK／原廠 INI，且仍然禁止在遊戲目錄建立 `backup/`。
...
### 2.8 玩家統計修改只走 `player.ini [gameN]`
遊戲 profile 頁的階級／軍事評價、單／多人戰績、時間、最愛國家／單位、資源、單位與儀式
統計來自連續的 `profiles\<玩家>\player.ini [game0]..[gameN]`，不是 `.adv`。實作必須遵守：

- 軍事評價依 Steam EXE 公式重建，階級名稱由遊戲依評價換算，不得偽造不存在的 rank 欄位。
- 保留 `[Player] hash`、未列入功能的 Player 欄位，以及仍存在之 game section 的未知欄位。
- 增加場數建立連續 section；減少場數會移除歷史 section，GUI 必須二次確認。
- 遊戲執行中 `stats get` 可唯讀，任何統計寫入一律拒絕。
- 不得為了這些統計去猜測、解壓或重封 `.adv`；`.adv` 內即時遊戲狀態仍不修改。
