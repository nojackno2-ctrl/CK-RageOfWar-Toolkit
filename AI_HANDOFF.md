...
> 🚀 **v1.0.5 發布準備完成（2026-09-04）**：
> - 版本號從 1.0.4 升為 **1.0.5**（`CKToolkit.csproj`、CLI `version`、三語 `strings.*.json` 全數同步）。
> - 包含近期重大改進：
>   - ISSUE-068：修改器執行期腳本通道（繞開 20 按鍵限制）。
>   - ISSUE-069~073：scoped hook 分流改比 player 索引、扣糧與訓練路徑補齊、多人守衛修正。
>   - ISSUE-074：遊戲設定分頁、維京領主與自由鬥士編入英雄隊伍功能。
> - 本地 `dotnet publish` 成功產生 `publish/fd`（~4.5MB 框架相依版）與 `publish/sc`（~53MB 獨立執行檔版）。
> - 完整 SelfTest 47 組測試全數通過，CLI smoke test 驗證合規。
