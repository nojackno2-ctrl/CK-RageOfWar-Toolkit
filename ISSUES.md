...
### ISSUE-078: 額外戰役／劇本對話與任務翻譯缺漏（修復長註解略過與 Mojibake 查表失敗，達成 100% 中文化）

- **問題編號**: `ISSUE-078`
- **提出日期**: 2026-09-05
- **狀態**: ⏳ **已修碼 · 待實測** (`Fixed - Pending Field Test`)
- **來源**: 使用者需求：「檢查戰役翻譯是不是全部都翻到」、「立即修復」
- **問題現象**:
  - 套用語言包後，原廠主戰役與 20 個遭遇戰劇本均已翻譯，但 5 套額外戰役/劇本（`Return to the Throne`, `Defenders`, `Invaders`, `The Fall of Avalon`, `Ascendency`）中，有大量戰役對話與任務說明（高達 356 條文字）依然顯示為英文或西班牙文（例如 Invaders 的 `ADVENTURE.LOC.XML` 呈現西班牙文 `Oleada tras oleada...`，Return to the Throne 的國王祭司對話 `CONV.XML` 221 條完全未翻）。
- **逆向分析與根因**:
  1. **長 Header 註解導致翻譯表識別失敗 (`LocXml.IsTranslationTable`)**:
     - `LocXml.cs` 判定某 XML 是否為 `<translationtable>` 時，原版寫法 `int n = Math.Min(data.Length, 64);` 僅檢查前 64 個位元組。
     - 額外戰役中有 4 個 XML 檔案在最開頭帶有作者宣告、授權資訊等長 XML 註解：
       - `Return to the Throne` 之 `CONV.XML`：開頭包含長達 1376 bytes 的大段歷史註解，`<translationtable>` 位於 offset 1376。
       - `Defenders` 之 `CONV.XML`、`SCENARIO.LOC.XML`、`ADVENTURE.LOC.XML`：註解約 101 bytes。
     - 前 64 bytes 找不到標籤，直接被 `IsTranslationTable` 判為 `false`，導致整份對話表被無條件略過、完全未進入字典查表替換。
  2. **模板解碼引號 Mojibake 導致字典查表脫靶 (`ExtraCampaignTemplates`)**:
     - `ExtraCampaignTemplates.cs` 內的 XML 模板在當初歷史提取自遊戲目錄或俄/西原版 PAK 時，UTF-8 引號位元組（`\xE2\x80\x99` 等）被以 Latin-1/Windows-1252 錯誤解碼為 `â\x80\x99`、`â\x80\x98`、`â\x80¦`、`Ã¢â\x82¬â\x84¢` 等 Mojibake 雜訊。
     - 語言包字典（`Translations.ByText`）存放的是標準 UTF-8 英文原句（如 `What's wrong with soldiers these days?` 或使用直引號 `'`、彎引號 `’`）。比對時雜訊字元破壞鍵值，導致字典查表直接失靶（MISS），回傳 null 並保留未翻譯文字。
  3. **彎引號（`’`）與直 ASCII 引號（`'`）互轉未對齊**:
     - 部份戰役 XML 使用直引號，而翻譯庫使用彎引號，或反之，先前缺乏雙向容錯機制。
- **修復方案**:
  1. **長 Header 識別擴展 (`LocXml.cs`)**:
     - 將 `LocXml.IsTranslationTable` 的搜尋長度由 64 位元組擴展至 4096 位元組（`Math.Min(data.Length, 4096)`）。4 個帶長註解的戰役 XML 100% 正確被辨識為翻譯表並納入翻譯重建。
  2. **模板雜訊清洗 (`ExtraCampaignTemplates.cs`)**:
     - 新增 `CleanMojibake` 函式，在 `GetTemplates()` 讀取時將 `â\u0080\u0099` 等 CP1252 誤解碼字元無失真還原為標準 Unicode 標點（`’`、`‘`、`…`、`–`、`—`、`“`、`”`）。
  3. **查表智慧容錯 (`Translations.cs`)**:
     - `Lookup` 查表邏輯新增雙向容錯：
       - 精確比對失敗時，自動將彎引號（`’` / `‘`）轉為 ASCII 直引號（`'`）再查一次；或將直引號轉為彎引號再查一次。
       - 自動容錯 Mojibake 標點，確保即使用戶自製或第三方舊版戰役 XML 也能平滑命中字典。
  4. **全語言包覆蓋與實機寫入**:
     - 額外戰役 118 個模板中所有 1003 個標籤（6 個為空，997 個有效條目）全數 100% 翻譯覆蓋。
     - 同步擴充至全 6 種語言包（`zh-TW`, `zh-CN`, `ja-JP`, `es-ES`, `it-IT`, `ru-RU`）。
