...
高於 2048 寬時的現況（同樣是 2026-08-21 實測，見 `CKPerf/hires.cpp` 與
`docs/hires-sidecar-audit.md`）：
- `CVXVisible`（引擎的可見性管理物件）在原版有一個 75 格內嵌陣列，容量不足時
  進關卡數秒內必定 crash（已有 first-chance fault report 佐證，根因是物件尾端
  `+0x4C0..+0x50F` 的 live bounds／容器指標被寫爆）。CKPerf 的 `hires.cpp` 以
  runtime code cave 把槽位定址重導向到外部 sidecar，解決了這個崩潰——2560x1440
  已可穩定進關卡、跑滿整場、零 crash report，多次實機驗證。
- 但**畫面本身仍未修好**：捲動鏡頭時，畫面 x≈2048 起會出現方向相依的貼圖塗抹／
  重複區塊。已排除的候選成因：sidecar 容量不足、consumer 端上限、引擎自身
  2048x2048 的區塊表面（已放大到 2560×2048 並以記憶體讀取確認生效，畫面仍未改善）。
  `SetDIBitsToDevice` 的實際 blit 幾何已確認是完整 2560 寬、正確送上螢幕，
  代表問題在更早的光柵化階段，真正成因**仍未定位**。
  `hires-sidecar-audit.md` 中一份較早的子代理報告曾宣稱 `CVXVisible+0x4C8`
  是一個 (left,top,right,bottom) 矩形容器——2026-08-21 傍晚以除錯器直接讀取執行中
  行程記憶體證明這個模型是錯的（該欄位其實是座標數值而非指標，真正的三個小型
  指標容器在 `+0x4E8`／`+0x4F4`／`+0x500`）。連帶地，當時算出的「1,171～1,286
  個矩形」與「sidecar 容量 2,342」等數字都是誤讀，不代表真實限制。
- **實務建議**：2560x1440／3840x2160 目前不應作為預設或推薦給終端使用者的選項；
  1920x1080（出廠預設）與 2048x1152（已驗證穩定）是目前唯一保證畫面正確的
  解析度。`hires.cpp` 的重導向修補本身無害（任一站點位元組不符即整組拒絕寫入，
  不影響 1920x1080 出廠路徑），予以保留以便未來繼續排查，但不應被誤認為
  「4K 已支援」。

工具本身保持通用：進階區塊仍接受任意表格容量與任意 `WxH`，使用者仍可手動選擇
高於 2048 的解析度，但 CLI/GUI 應清楚標示畫面尚未修好。
