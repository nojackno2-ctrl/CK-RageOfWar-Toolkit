...
public static partial class CliHost
{
    ...
    /// <summary>
    /// <c>profile</c>：一次跑完整場診斷——注入層 + 取樣／偵錯層，同一個 pid、同一個輸出資料夾。
    ///
    /// 跟 GUI 的分析器分頁走的是同一條 <see cref="DiagnosticSession"/>，所以兩邊不會再各自
    /// 記到一半。<c>--mode</c> 對應分頁上「怎麼開始」那三個選項；<c>--no-inject</c> 是給
    /// 「只要外部取樣器、不要碰遊戲行程」這種對照組用的。
    /// </summary>
